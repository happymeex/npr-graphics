...
Image ApplyLayer(const Image &image1, const Image &image2, const std::function<glm::vec3(glm::vec3, glm::vec3)> &function)
{
...
Image ApplyFilter(const Image &image, const glm::mat3 &filter, bool on_transparency = false)
{
            ...
            if (on_transparency)
            {
                ...
                for (int i = -1; i <= 1; ++i)
                {
                    for (int j = -1; j <= 1; ++j)
                    {
                        ...
                        pixel += filter[i + 1][j + 1] * image.GetPixel(true_x, true_y);
                    }
                }
            ...
            else
            {
                ...
                for (int i = -1; i <= 1; ++i)
                {
                    for (int j = -1; j <= 1; ++j)
                    {
                        auto true_x = std::min(std::max(0, x + i), upper_bound);
                        auto true_y = std::min(std::max(0, y + j), lower_bound);
                        pixel += filter[i + 1][j + 1] * glm::vec3(image.GetPixel(true_x, true_y));
                    }
                }
...
Image GetEdges(const Image &image)
{
    ...
    // return ApplyLayer(ApplyFilter(image, GX), ApplyFilter(image, GY), magnitude);
    auto result = ApplyLayer(ApplyFilter(image, GX), ApplyFilter(image, GY), magnitude);
...
glm::vec3 Tracer::TraceRay(const Ray &ray, int bounces, HitRecord &hit_record, RenderStyle style)
{
    ...
    for (const auto &light : scene_->lights)
    {
        ...
        color += light_intensity * specular_scalar * specular_color;
    }
